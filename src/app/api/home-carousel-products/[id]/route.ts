...
// DELETE /api/home-carousel-products/[id] - Listeden cikar (sadece admin)
export async function DELETE(_request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const { error } = await supabase.from('home_carousel_products').delete().eq('id', params.id)
    if (error) return NextResponse.json({ success: false, error: error.message }, { status: 500 })
    return NextResponse.json({ success: true })
  } catch (err) {
    console.error('API home-carousel-products DELETE:', err)
