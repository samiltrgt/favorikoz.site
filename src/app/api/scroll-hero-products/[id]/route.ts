...
// DELETE /api/scroll-hero-products/[id] - Listeden çıkar (sadece admin)
export async function DELETE(_request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const { error } = await supabase.from('scroll_hero_products').delete().eq('id', params.id)

    if (error) return NextResponse.json({ success: false, error: error.message }, { status: 500 })
    return NextResponse.json({ success: true })
  } catch (err) {
    console.error('API scroll-hero-products DELETE:', err)
