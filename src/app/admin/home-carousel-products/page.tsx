...
export default function HomeCarouselProductsAdminPage() {
  const [searchTerm, setSearchTerm] = useState('')
  const [allProducts, setAllProducts] = useState<Product[]>([])
  const [carouselItems, setCarouselItems] = useState<HomeCarouselItem[]>([])
  ...
  const loadData = async () => {
    try {
      const [productsRes, carouselRes] = await Promise.all([
        fetch('/api/products?scope=admin'),
        fetch('/api/home-carousel-products?scope=admin'),
      ])
      const productsData = await productsRes.json()
      const carouselData = await carouselRes.json()
      if (productsData.success) setAllProducts(productsData.data || [])
      if (carouselData.success) setCarouselItems(carouselData.data || [])
    } catch (e) {
  ...
  const currentProductIds = carouselItems.map((item) => item.product_id)

  const addToCarousel = async (productId: string) => {
    try {
      const res = await fetch('/api/home-carousel-products', {
        method: 'POST',
      ...
      console.error('Error adding home carousel item:', e)
    }
  }

  const removeFromCarousel = async (itemId: string) => {
    try {
      const res = await fetch(`/api/home-carousel-products/${itemId}`, { method: 'DELETE' })
      if (res.ok) await loadData()
    } catch (e) {
      console.error('Error removing home carousel item:', e)
    }
  }
          ...
          <div className="flex items-center">
            ...
            <div>
              <h2 className="text-2xl font-bold leading-7 text-gray-900 sm:truncate sm:text-3xl sm:tracking-tight">
                Ana Sayfa Urun Slideri
              </h2>
              <p className="mt-1 text-sm text-gray-500">
                Scroll hero altindaki urun sliderini buradan yonetebilirsiniz.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white shadow rounded-lg">
          <div className="px-6 py-4 border-b border-gray-200">
            <h3 className="text-lg font-medium text-gray-900">Secili Urunler ({carouselItems.length})</h3>
          </div>
          <div className="p-6">
              ...
              {filteredProducts.map((product) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => !isSelected && addToCarousel(product.id)}
                  >
                    <div className="flex-shrink-0">
                      <Image
                        src={product.image || '/placeholder.png'}
